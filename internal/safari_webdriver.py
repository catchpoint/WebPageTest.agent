...
class SafariWebDriver(DesktopBrowser):
    """Safari"""
    ...
    def prepare(self, job, task):
        ...
        if not task['cached'] and os.path.isdir(profile_template):
            try:
                ...
                logging.error('Error copying Safari profile')
        # Delete any unsent crash reports
        crash_dir = None
        # Only support darwin, macOS platform
        crash_dir = os.path.join(os.path.expanduser('~'), 'Library', 'Logs', 'CrashReporter')
    ...
    def disable_fsync(self, command_line):
        """Use eatmydata, on macOS, to disable fsync"""
        try:
            cmd = ['eatmydata', 'data']
    ...
    def start_safari(self, job, task):
        ...
        self.webdriver = webdriver.Safari(desired_capabilities=capabilities)
        logging.debug('Resizing browser to %dx%d', task['width'], task['height'])
        self.webdriver.set_window_rect(x=0,y=0,height=task['height'], width=task['width'])
        capabilities = self.webdriver.capabilities
        self.browser_version = capabilities['browserVersion']
        self.webdriver.get(self.start_page)

    def launch(self, job, task):
        """Launch the browser"""
        if self.job['message_server']:
            self.job['message_server'].flush_messages()
        self.connected = False
        logging.debug('launching safari')

        try:
            self.start_safari(job, task)
            time.sleep(0.5)
            # There is some security policy that prevents the extension
            # to talk to a local http server when rendering 127.0.0.1:8888/orange.html.
            # Once it navigates to a different site, it allows the extension access.
            # So just mark this as connected for Desktop Safari browser.
            self.connected = True
            if self.connected:
                ...
                if ua_string and 'AppendUA' in task:
                ...
                if 'lat' in self.job and 'lng' in self.job:
                    try:
                        ...
                        logging.error('Error overriding location')
            ...
            logging.error('Error starting Safari')
    ...
    def configure_prefs(self):
        ...
        prefs_file = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                                  'support', 'Safari', 'profile', 'prefs.js')
    ...
    def driver_set_pref(self, key, value):
        """Set a Safari pref at runtime using Webdriver"""
        # TODO(wuwang): add set_pref for Safari
        pass

    def set_window_size(self, width, height):
        """Position the window"""
        self.webdriver.set_window_rect(x=0, y=0, height=height, width=width)
        ...
        if self.connected:
            ...
            while len(task['script']) and task['error'] is None and monotonic() < end_time and not self.must_exit:
                ...
                try:
                    ...
                    logging.error("Exception running task")
                if command['record']:
            ...
            if self.webdriver:
                try:
                    ...
                    logging.error('Webdriver exception navigating to about:blank after the test')
            self.task = None
        ...
        if self.job['message_server'] is not None:
            ...
            while monotonic() < end_time and not self.connected and not self.must_exit:
                try:
                    ...
                    try:
                        logging.debug('get_message returns: {}'.format(message))
    ...
    def execute_js(self, script):
        ...
        if self.webdriver:
            try:
                ret = self.webdriver.execute_script(script)
            except Exception:
    ...
    def run_js_file(self, file_name):
        ...
        if script is not None:
            try:
                ret = self.webdriver.execute_script('return ' + script)
            except Exception:
                logging.error('Error executing script file')
            if ret is not None:
                logging.debug(ret)
        return ret
            ...
            for name in self.job['customMetrics']:
                ...
                if custom_script.find('$WPT_REQUESTS') >= 0:
                    ...
                    try:
                        ...
                        logging.error('Error substituting request data into custom script')
                if custom_script.find('$WPT_BODIES') >= 0:
                    ...
                    try:
                        ...
                        logging.error('Error substituting request data with bodies into custom script')
                ...
                try:
                    ...
                    logging.error('Error collecting custom metrics')
            ...
            if message == 'onBeforeNavigate':
                ...
                if 'injectScript' in self.job and self.connected:
                    ...
                    try:
                        ...
                        logging.error('Error injecting script')
    ...
    def on_stop_recording(self, task):
        ...
        try:
            ...
            logging.error("Error writing the long tasks")
        try:
            interactive_periods = []
            last_end = 0
            ...
            logging.error("Error writing the interactive periods")
    ...
    def navigate(self, url):
        ...
        if self.webdriver:
            try:
                ...
                logging.error(('Error navigating Safari: %s', str(err)))
    ...
    def set_pref(self, key, value_str):
        ...
        if value is not None:
            try:
                ...
                logging.error('Error setting pref')
        ...
        if self.connected:
            try:
                ...
                if png:
                    ...
                    tmp_file = path + '.png'
                    self.webdriver.save_screenshot(tmp_file)
                ...
                logging.error('Exception grabbing screen shot: %s', str(err))
    ...
    def merge_requests(self, request_timings):
                        ...
                        for header in req['response_headers']:
                            if 'name' in header and 'value' in header:
                                try:
                                    ...
                                    logging.error('Error appending response header')
                    if 'created' in req:
        ...
        for request in requests:
            for req in request_timings:
                try:
                    ...
                    logging.error('Error populating request')
        ...
        for req in request_timings:
            try:
                ...
                logging.error('Error adding request from logs')
        ...
        for request in requests:
            try:
                ...
                logging.error('Error processing headers')
