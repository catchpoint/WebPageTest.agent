...
class DevTools(object):
            ...
            if "trace" in self.job and self.job["trace"]:
                ...
                if "traceCategories" in self.job:
                    ...
                    for category in categories:
                        if (
                            category.find("*") < 0
                            and category not in trace_config["includedCategories"]
                        ):
            ...
            if "timeline" in self.job and self.job["timeline"]:
                ...
                if "timeline_fps" in self.job and self.job["timeline_fps"]:
                    if (
                        ...
                        not in trace_config["includedCategories"]
                    ):
                        trace_config["includedCategories"].append(
                            "disabled-by-default-devtools.timeline"
                        )
                    if (
    ...
    def disable_cache(self, disable):
        ...
        self.send_command(
            "Network.setCacheDisabled", {"cacheDisabled": disable}, wait=True
        )
