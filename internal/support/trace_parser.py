...
class Trace:
                ...
                for thread in main_threads:
                    try:
                        ...
                        if thread in self.cpu["slices"]:
                            for name in self.cpu["slices"][thread].keys():
                                for slice in range(
                                    len(self.cpu["slices"][thread][name])
                                ):
                                    thread_cpu += self.cpu["slices"][thread][name][
                                        slice
                                    ]
        ...
        if "source_dependency" in params and "id" in params["source_dependency"]:
            ...
            if (
                ...
                if (
                    ...
                    self.netlog["urlrequest"][entry["urlrequest"]][
                        "h2_session"
                    ] = h2_session_id
    ...
    def ProcessNetlogSocketEvent(self, trace_event):
        ...
        if (
            "ssl_start" not in entry
            and name == "SSL_CONNECT"
            and trace_event["ph"] == "b"
        ):
    ...
    def ProcessNetlogUrlRequestEvent(self, trace_event):
        ...
        if (
            "start" not in entry
            and name == "HTTP_TRANSACTION_SEND_REQUEST"
            and trace_event["ph"] == "e"
        ):
            entry["start"] = trace_event["ts"]
                    ...
                    if (
                        ...
                        for stat in trace_event["args"]["runtime-call-stats"]:
                            if (
                                ...
                                self.v8stats[thread][name]["events"][stat]["dur"] += (
                                    float(
