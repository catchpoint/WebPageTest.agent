...
class DevToolsParser(object):
        ...
        if raw_events is not None and len(raw_events):
                ...
                if "method" in raw_event and "params" in raw_event:
                    ...
                    if method == "Debugger.scriptParsed" and "scriptId" in params:
                        ...
                        if script_id not in self.script_ids:
                            if (
                                "stackTrace" in params
                                and "callFrames" in params["stackTrace"]
                            ):
                                for frame in params["stackTrace"]["callFrames"]:
                                    if "url" in frame and frame["url"]:
                                        ...
                                        if (
                                            "scriptId" in frame
                                            and frame["scriptId"]
                                            and frame["scriptId"] not in self.script_ids
                                        ):
                                            self.script_ids[
                                                frame["scriptId"]
                                            ] = script_url
                        ...
                        if (
                            ...
                            if request_id in raw_requests:
                                if "redirectResponse" in params:
                                    ...
                                    raw_requests[request_id]["fromNet"] = False
                                    if (
                                        "fromDiskCache" in params["redirectResponse"]
                                        and not params["redirectResponse"][
                                            "fromDiskCache"
                                        ]
                                        and "headers" in raw_requests[request_id]
                                        and len(raw_requests[request_id]["headers"])
                                    ):
                                        raw_requests[request_id]["fromNet"] = True
                                    raw_requests[request_id]["response"] = params[
                                        "redirectResponse"
                                    ]
                                count = 0
                            ...
                            if (
                                "endTime" not in request
                                or timestamp > request["endTime"]
                            ):
                            ...
                            if method == "Network.dataReceived":
                                ...
                                if (
                                    "encodedDataLength" in params
                                    and params["encodedDataLength"]
                                ):
                                    if "bytesFinished" not in request:
                                        request["bytesInEncoded"] += params[
                                            "encodedDataLength"
                                        ]
                            ...
                            if (
                                ...
                                and "response" in params
                            ):
                                if (
                                    not has_request_headers
                                    and "requestHeaders" in params["response"]
                                ):
                                ...
                                if (
                                    "fromCache" in request
                                    and "timing" in params["response"]
                                ):
                                ...
                                if (
                                    "fromDiskCache" in params["response"]
                                    and not params["response"]["fromDiskCache"]
                                    and "headers" in request
                                    and len(request["headers"])
                                ):
                            ...
                            if (
                                ...
                                and "response" not in request
                                and (
                                    "fromCache" not in request
                                    or not request["fromCache"]
                                )
                            ):
                                if "blockedReason" not in params and (
                                    ...
                                    if "errorText" in params and params[
                                        "errorText"
                                    ].find("ERR_CONNECTION_REFUSED"):
                                        request["fromNet"] = False
                                    else:
                                        request["fromNet"] = True
                    ...
                    if (
                        ...
                        and "timestamp" in params
                        and "domContentLoadedEventStart" not in page_data
                    ):
            ...
            for request_id in raw_requests:
                ...
                if (
                    ...
                    and "timing" in request["response"]
                    and "startTime" in request
                ):
                    min_time = None
                    ...
                    if (
                        "startTime" not in page_data
                        or request["startTime"] < page_data["startTime"]
                    ):
    ...
    def process_requests(self, raw_requests, raw_page_data):
        ...
        if "onload" in raw_page_data:
            page_data["loadTime"] = int(
                round(raw_page_data["onload"] - raw_page_data["startTime"])
            )
        ...
        if "domContentLoadedEventStart" in raw_page_data:
            page_data["domContentLoadedEventStart"] = int(
                round(
                    raw_page_data["domContentLoadedEventStart"]
                    - raw_page_data["startTime"]
                )
            )
            if "domContentLoadedEventEnd" in raw_page_data:
                page_data["domContentLoadedEventEnd"] = int(
                    round(
                        raw_page_data["domContentLoadedEventEnd"]
                        - raw_page_data["startTime"]
                    )
                )
            else:
                page_data["domContentLoadedEventEnd"] = page_data[
                    "domContentLoadedEventStart"
                ]
        if "loadEventStart" in raw_page_data:
            page_data["loadEventStart"] = int(
                round(raw_page_data["loadEventStart"] - raw_page_data["startTime"])
            )
            if "loadEventEnd" in raw_page_data:
                page_data["loadEventEnd"] = int(
                    round(raw_page_data["loadEventEnd"] - raw_page_data["startTime"])
                )
            else:
        ...
        for raw_request in raw_requests:
            if "url" in raw_request:
                ...
                request["method"] = (
                    raw_request["method"] if "method" in raw_request else ""
                )
                ...
                if (
                    "response" in raw_request
                    and "timing" in raw_request["response"]
                    and "sendStart" in raw_request["response"]["timing"]
                    and raw_request["response"]["timing"]["sendStart"] >= 0
                ):
                    start_time = raw_request["response"]["timing"]["sendStart"]
                    if (
                        "fullyLoaded" not in page_data
                        or start_time > page_data["fullyLoaded"]
                    ):
                ...
                if "endTime" in raw_request:
                    ...
                    if (
                        "fullyLoaded" not in page_data
                        or end_time > page_data["fullyLoaded"]
                    ):
                        page_data["fullyLoaded"] = int(round(end_time))
                request["ttfb_ms"] = -1
                if "firstByteTime" in raw_request:
                    request["ttfb_ms"] = int(
                        round(raw_request["firstByteTime"] - raw_request["startTime"])
                    )
                request["load_start"] = int(
                    round(start_time - raw_page_data["startTime"])
                )
                ...
                if (
                    "bytesInEncoded" in raw_request
                    and raw_request["bytesInEncoded"] > 0
                ):
                    request["objectSize"] = str(
                        int(round(raw_request["bytesInEncoded"]))
                    )
                    request["bytesIn"] = int(round(raw_request["bytesInEncoded"]))
                if "bytesInData" in raw_request:
                    if request["objectSize"] == "":
                        request["objectSize"] = str(
                            int(round(raw_request["bytesInData"]))
                        )
                    if request["bytesIn"] == 0:
                        request["bytesIn"] = int(round(raw_request["bytesInData"]))
                    request["objectSizeUncompressed"] = int(
                        round(raw_request["bytesInData"])
                    )
                ...
                if (
                    request["bytesIn"] == 0
                    and "response" in raw_request
                    and "headers" in raw_request["response"]
                    and "Content-Length" in raw_request["response"]["headers"]
                ):
                    request["bytesIn"] = int(
                        re.search(
                            r"\d+", raw_request["response"]["headers"]["Content-Length"]
                        ).group()
                    )
                request["expires"] = (
                    self.get_response_header(raw_request, "Expires")
                    .replace("\n", ", ")
                    .replace("\r", "")
                )
                request["cacheControl"] = (
                    self.get_response_header(raw_request, "Cache-Control")
                    .replace("\n", ", ")
                    .replace("\r", "")
                )
                request["contentType"] = self.get_response_header(
                    raw_request, "Content-Type"
                ).split(";")[0]
                request["contentEncoding"] = (
                    self.get_response_header(raw_request, "Content-Encoding")
                    .replace("\n", ", ")
                    .replace("\r", "")
                )
                object_size = (
                    self.get_response_header(raw_request, "Content-Length")
                    .split("\n")[0]
                    .replace("\r", "")
                )
                ...
                if len(request["objectSize"]):
                    request["objectSize"] = int(
                        re.search(r"\d+", request["objectSize"]).group()
                    )
                request["socket"] = -1
                if (
                    "response" in raw_request
                    and "connectionId" in raw_request["response"]
                ):
                    request["socket"] = raw_request["response"]["connectionId"]
                if (
                    "response" in raw_request
                    and "remoteIPAddress" in raw_request["response"]
                ):
                ...
                if "response" in raw_request and "timing" in raw_request["response"]:
                    ...
                    if (
                        ...
                        and timing["receiveHeadersEnd"] >= timing["sendStart"]
                    ):
                        request["ttfb_ms"] = int(
                            round(timing["receiveHeadersEnd"] - timing["sendStart"])
                        )
                        if request["load_ms"] >= 0:
                            request["load_ms"] = max(
                                request["ttfb_ms"], request["load_ms"]
                            )
                    ...
                    if request["socket"] != -1 and request["socket"] not in connections:
                        ...
                        if "dnsStart" in timing and "dnsStart" >= 0:
                            ...
                            if dns_key not in dns_times:
                                ...
                                request["dns_start"] = int(
                                    round(
                                        timing["dnsStart"] - raw_page_data["startTime"]
                                    )
                                )
                        ...
                        if "connectStart" in timing and timing["connectStart"] >= 0:
                            request["connect_start"] = int(
                                round(
                                    timing["connectStart"] - raw_page_data["startTime"]
                                )
                            )
                ...
                if "initiator" in raw_request:
                    ...
                    if "url" in raw_request["initiator"]:
                        ...
                        if "lineNumber" in raw_request["initiator"]:
                            request["initiator_line"] = raw_request["initiator"][
                                "lineNumber"
                            ]
                    elif (
                        "stack" in raw_request["initiator"]
                        and "callFrames" in raw_request["initiator"]["stack"]
                        and raw_request["initiator"]["stack"]["callFrames"]
                    ):
                        for frame in raw_request["initiator"]["stack"]["callFrames"]:
                            if "url" in frame and frame["url"]:
                                ...
                                if "functionName" in frame and frame["functionName"]:
                                    request["initiator_function"] = frame[
                                        "functionName"
                                    ]
                                break
                            elif (
                                "scriptId" in frame
                                and frame["scriptId"]
                                and frame["scriptId"] in self.script_ids
                            ):
                                request["initiator"] = self.script_ids[
                                    frame["scriptId"]
                                ]
                                break
                ...
                if (
                    "response" in raw_request
                    and "requestHeadersText" in raw_request["response"]
                ):
                    for line in raw_request["response"][
                        "requestHeadersText"
                    ].splitlines():
                    ...
                    "response" in raw_request
                    and "requestHeaders" in raw_request["response"]
                ):
                    for key in raw_request["response"]["requestHeaders"]:
                        for value in raw_request["response"]["requestHeaders"][
                            key
                        ].splitlines():
                            try:
                ...
                if (
                    "response" in raw_request
                    and "headersText" in raw_request["response"]
                ):
                    ...
                    for key in raw_request["response"]["headers"]:
                        for value in raw_request["response"]["headers"][
                            key
                        ].splitlines():
                            try:
                ...
                if "startTime" in raw_request:
                    start_offset = int(
                        round(raw_request["startTime"] - raw_page_data["startTime"])
                    )
                    if (
                        "fullyLoaded" not in page_data
                        or start_offset > page_data["fullyLoaded"]
                    ):
                        page_data["fullyLoaded"] = start_offset
                if "endTime" in raw_request:
                    end_offset = int(
                        round(raw_request["endTime"] - raw_page_data["startTime"])
                    )
                    if (
                        "fullyLoaded" not in page_data
                        or end_offset > page_data["fullyLoaded"]
                    ):
    ...
    def process_netlog_requests(self):
        ...
        if self.netlog_requests_file is not None and os.path.isfile(
            self.netlog_requests_file
        ):
                ...
                if "full_url" in request:
                    ...
                    for entry in netlog:
                        if (
                            "url" in entry
                            and "start" in entry
                            and "claimed" not in entry
                            and entry["url"] == request["full_url"]
                        ):
                            entry["claimed"] = True
                            for key in mapping:
                                try:
                                    if key in entry:
                                        if type(entry[key]) is list:
                                            ...
                                            r"^\d+\.?(\d+)?$", str(entry[key]).strip()
                                        ):
                                            request[mapping[key]] = int(
                                                round(float(str(entry[key]).strip()))
                                            )
                                        else:
                                    ...
                                    pass
                            if "start" in entry:
                                request["load_start_float"] = float(
                                    str(entry["start"]).strip()
                                )
                            if "certificates" in entry:
                                request["certificates"] = entry["certificates"]
                            if "first_byte" in entry:
                                request["ttfb_ms"] = int(
                                    round(entry["first_byte"] - entry["start"])
                                )
                            if "end" in entry:
                                request["load_ms"] = int(
                                    round(entry["end"] - entry["start"])
                                )
                            ...
                            if "response_headers" in entry:
                                ...
                                for header in entry["response_headers"]:
                                    ...
                                    if matches:
                                        request["responseCode"] = int(matches.group(1))
                                    matches = re.search(
                                        r"^content-type: (.+)", header, re.IGNORECASE
                                    )
                                    if matches:
                                        request["contentType"] = matches.group(1).split(
                                            ";"
                                        )[0]
                                    matches = re.search(
                                        r"^cache-control: (.+)", header, re.IGNORECASE
                                    )
                                    if matches:
            ...
            for entry in netlog:
                if "claimed" not in entry and "url" in entry and "start" in entry:
                    ...
                    if "end" in entry:
                        request["load_ms"] = int(round(entry["end"] - entry["start"]))
                    if "pushed" in entry and entry["pushed"]:
                        request["was_pushed"] = 1
                    if "start" in entry:
                    ...
                    if "response_headers" in entry:
                        ...
                        for header in entry["response_headers"]:
                            ...
                            if matches:
                                request["cacheControl"] = matches.group(1)
                            matches = re.search(
                                r"^content-encoding: (.+)", header, re.IGNORECASE
                            )
                            if matches:
                                request["contentEncoding"] = matches.group(1)
                            matches = re.search(
        ...
        if "main_frame" in page_data:
            ...
            for request in requests:
                if (
                    ...
                    and len(request["full_url"])
                    and "frame_id" in request
                    and request["frame_id"] == page_data["main_frame"]
                    ...
                    if "contentType" not in request or (
                        request["contentType"].find("ocsp-response") < 0
                        and request["contentType"].find("ca-cert") < 0
                    ):
    ...
    def process_page_data(self):
        ...
        for request in requests:
            if (
                "TTFB" not in page_data
                and "load_start" in request
                and "ttfb_ms" in request
                and request["ttfb_ms"] >= 0
                and "responseCode" in request
                ...
                if "contentType" not in request or (
                    ...
                    page_data["TTFB"] = int(
                        round(request["load_start"] + request["ttfb_ms"])
                    )
                    if (
                        request["ssl_end"] >= request["ssl_start"]
                        and request["ssl_start"] >= 0
                    ):
                        page_data["basePageSSLTime"] = int(
        ...
        if page_data["responses_200"] == 0:
            if (
                len(requests) > 0
                and "responseCode" in requests[0]
                and requests[0]["responseCode"] >= 400
            ):
                ...
                if (
                    ...
                    if main_frame is not None and event["args"]["frame"] == main_frame:
                        if navigation_start is None:
                            ...
                            elapsed = int(
                                round(float(event["ts"] - navigation_start) / 1000.0)
                            )
                            for name in names:
                                if event["name"] == name:
    ...
    def process_cpu_times(self):
        ...
        try:
            ...
            if (
                ...
                and self.cpu_times is not None
                and os.path.isfile(self.cpu_times)
            ):
                ...
                if (
                    cpu
                    and "main_thread" in cpu
                    and "slices" in cpu
                    and cpu["main_thread"] in cpu["slices"]
                    and "slice_usecs" in cpu
                ):
                    busy = 0
                    busy_doc = 0
                    ...
                    for name in all_slices:
                        ...
                        last_slice = min(
                            int(math.ceil((end * 1000) / usecs)), len(slices)
                        )
                    ...
                    for category in ["cpuTimes", "cpuTimesDoc"]:
                        for name in page_data[category]:
                            page_data[category][name] = int(
                                round(page_data[category][name])
                            )
            ...
            if os.path.isfile(self.v8_stats):
                ...
                if stats and "main_threads" in stats and "threads" in stats:
                    ...
                    for thread in stats["threads"]:
                        group = (
                            "main_thread" if thread in main_threads else "background"
                        )
                        for category in stats["threads"][thread]:
                            prefix = (
                                ""
                                if category == "V8.RuntimeStats"
                                else "{0}.".format(category)
                            )
                            if (
                                "dur" in stats["threads"][thread][category]
                                and "breakdown" in stats["threads"][thread][category]
                            ):
                                remainder = stats["threads"][thread][category]["dur"]
                                for event in stats["threads"][thread][category][
                                    "breakdown"
                                ]:
                                    detail = stats["threads"][thread][category][
                                        "breakdown"
                                    ][event]
                                    if "dur" in detail:
                                        ...
                                        page_data["v8Stats"][group][name] += detail[
                                            "dur"
                                        ]
                                        remainder -= detail["dur"]
                                if remainder > 0.0:
                                    page_data["v8Stats"][group][
                                        "{0}unaccounted".format(prefix)
                                    ] = remainder
        except Exception:
            pass
        pass
...
def main():
    ...
    parser.add_argument(
        "-p", "--optimization", help="Input optimization results file (optional)."
    )
