...
class WebPageTest(object):
    ...
    def __init__(self, options, workdir):
        ...
        try:
            ...
            out = subprocess.check_output(
                "git log -1 --format=%cd --date=raw", shell=True, cwd=directory
            )
            if out is not None:
    ...
    def benchmark_cpu(self):
        ...
        if not self.options.android and not self.options.iOS:
            ...
            logging.debug(
                "CPU Benchmark elapsed time: %0.3f, multiplier: %0.3f",
                elapsed,
                self.cpu_scale_multiplier,
            )
    ...
    def load_from_ec2(self):
        ...
        if platform.system() == "Linux":
            subprocess.call(
                ["sudo", "route", "add", "169.254.169.254", "gw", "127.0.0.1", "lo"]
            )
            ...
            try:
                ...
                if len(response.text):
                    ...
                    if job["type"] == "lighthouse":
                        job["fvonly"] = 1
                        job["lighthouse"] = 1
                    job["keep_lighthouse_trace"] = bool(
                        "lighthouseTrace" in job and job["lighthouseTrace"]
                    )
                    job["lighthouse_throttle"] = bool(
                        "lighthouseThrottle" in job and job["lighthouseThrottle"]
                    )
                    job["video"] = bool("Capture Video" in job and job["Capture Video"])
                    job["keepvideo"] = bool("keepvideo" in job and job["keepvideo"])
                    job["disable_video"] = bool(
                        not job["video"]
                        and "disable_video" in job
                        and job["disable_video"]
                    )
                    job["interface"] = None
                    job["persistent_dir"] = self.persistent_dir
                    if "throttle_cpu" in job:
                        throttle = float(
                            re.search(r"\d+\.?\d*", str(job["throttle_cpu"])).group()
                        )
                        if (
                            "bypass_cpu_normalization" not in job
                            or not job["bypass_cpu_normalization"]
                        ):
        ...
        if "current_state" not in job or not job["current_state"]["done"]:
            if "run" in job:
                ...
                if "current_state" not in job:
                    job["current_state"] = {
                        "run": int(re.search(r"\d+", str(job["run"])).group()),
                        "repeat_view": False,
                        "done": False,
                    }
            ...
            if job["current_state"]["run"] <= job["runs"]:
                ...
                if "debug" in job and job["debug"]:
                    task["debug_log"] = os.path.join(
                        task["dir"], task["prefix"] + "_debug.log"
                    )
                    try:
                ...
                if "mobile" in job and job["mobile"]:
                    if "browser" in job and job["browser"] in self.margins:
                        task["width"] = job["width"] + max(
                            self.margins[job["browser"]]["width"], 0
                        )
                        task["height"] = job["height"] + max(
                            self.margins[job["browser"]]["height"], 0
                        )
                    else:
                ...
                if "thumbsize" not in job and (
                    task["width"] < 600 or task["height"] < 600
                ):
                    job["fullSizeVideo"] = 1
    ...
    def build_script(self, job, task):
        ...
        if "script" in job:
                        ...
                        if target is not None and value is not None:
                            ...
                            if (
                                ...
                                if "mobile" not in job or not job["mobile"]:
                                    if (
                                        "browser" in job
                                        and job["browser"] in self.margins
                                    ):
                                        job["width"] += max(
                                            self.margins[job["browser"]]["width"], 0
                                        )
                        ...
                        if (
                            ...
                            if target.find('"') == -1 and value.find('"') == -1:
                                ...
                                if "host_rules" not in task:
                                    task["host_rules"] = []
                                task["host_rules"].append(
                                    '"MAP {0} {1}"'.format(target, value)
                                )
                                if re.match(
                                    r"^\d+\.\d+\.\d+\.\d+$", value
                                ) and re.match(r"^[a-zA-Z0-9\-\.]+$", target):
                        ...
                        if (
                            ...
                            if addr is not None and target.find('"') == -1:
                                ...
                                if "host_rules" not in task:
                                    task["host_rules"] = []
                                task["host_rules"].append(
                                    '"MAP {0} {1}"'.format(target, addr)
                                )
                                if re.match(r"^\d+\.\d+\.\d+\.\d+$", addr) and re.match(
                                    r"^[a-zA-Z0-9\-\.]+$", target
                                ):
                        ...
                        if target is not None:
                            ...
                            if separator >= 0:
                                ...
                                if command in ["click", "sendclick"]:
                                    ...
                                    command in ["setvalue", "selectvalue"]
                                    and value is not None
                                ):
                                    script += '.value="{0}";'.format(
                                        value.replace('"', '\\"')
                                    )
                                elif command == "setinnertext" and value is not None:
                                    script += '.innerText="{0}";'.format(
                                        value.replace('"', '\\"')
                                    )
                                elif command == "setinnerhtml" and value is not None:
                                    script += '.innerHTML="{0}";'.format(
                                        value.replace('"', '\\"')
                                    )
                                command = "exec"
                                target = script
            ...
            record_count += 1
            task["script"].append(
                {"command": "navigate", "target": job["url"], "record": True}
            )
    ...
    def update_browser_viewport(self, task):
        ...
        if (
            "actual_viewport" in task
            and "width" in task
            and "height" in task
            and self.job is not None
            and "browser" in self.job
        ):
            ...
            if (
                browser not in self.margins
                or self.margins[browser]["width"] != width
                or self.margins[browser]["height"] != height
            ):
        ...
        try:
            while True:
                ...
                try:
                    ...
                    response = session.get(
                        url, headers=headers, stream=True, timeout=30, proxies=proxies
                    )
            ...
            if requests and "requests" in requests:
                ...
                for request in requests["requests"]:
                    if (
                        ...
                        and request["full_url"].find("ocsp") == -1
                        and request["full_url"].find(".woff") == -1
                        and request["full_url"].find(".ttf") == -1
                        and "contentType" in request
                    ):
                        ...
                        if need_body:
                            ...
                            if body_id not in bodies:
                                ...
                                if (
                                    "headers" in request
                                    and "request" in request["headers"]
                                ):
            ...
            if count:
                    ...
                    while True:
                        ...
                        if os.path.isfile(task["file"]):
                            ...
                            try:
                                ...
                                body_index += 1
                                file_name = "{0:03d}-{1}-body.txt".format(
                                    body_index, task["id"]
                                )
                ...
                if bodies:
                    with zipfile.ZipFile(
                        bodies_zip, "a", zipfile.ZIP_DEFLATED
                    ) as zip_file:
                        for body in bodies:
            ...
            if os.path.isdir(task["dir"]):
                            ...
                            for filename in os.listdir(video_dir):
                                ...
                                if os.path.isfile(filepath):
                                    ...
                                    if os.path.getsize(filepath) > 100000:
                                        ...
                                        if self.post_data(
                                            self.url + "resultimage.php",
                                            data,
                                            filepath,
                                            task["prefix"] + "_" + filename,
                                        ):
                                            os.remove(filepath)
                                        else:
                                            needs_zip.append(
                ...
                for filename in os.listdir(task["dir"]):
                    ...
                    if os.path.isfile(filepath):
                        ...
                        if (
                            not self.job["keepvideo"]
                            and filename[-4:] == ".mp4"
                            and filename.find("rendered_video") == -1
                        ):
                            try:
                                os.remove(filepath)
                            except Exception:
                            ...
                            if self.post_data(
                                self.url + "resultimage.php", data, filepath, filename
                            ):
                                try:
                                    os.remove(filepath)
                                except Exception:
    ...
    def post_data(self, url, data, file_path, filename):
        ...
        try:
            if file_path is not None and os.path.isfile(file_path):
                self.session.post(
                    url, files={"file": (filename, open(file_path, "rb"))}, timeout=300
                )
            else:
