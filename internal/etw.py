...
class ETW(object):
    """Handle ETW traces for Edge and IE"""
    ...
    def start_recording(self, log_file):
        ...
        if len(self.kernel_categories) or len(self.user_categories):
            ...
            logging.debug('Capturing ETW trace {0} to "{1}"'.format(self.trace_name, log_file))
    ...
    def process(self, task):
        requests = None
        if self.log_file is not None and self.started:
            csv_file = self.log_file + '.csv'
            self.extract_csv(csv_file)
            if os.path.exists(csv_file):
                logging.debug('Parsing Events')
    ...
    def extract_csv(self, csv_file):
        ...
        if self.log_file is not None:
            logging.debug('Converting ETW trace to CSV')
            command = ['xperf', '-i', self.log_file, '-o',
                       csv_file, '-target', 'machine', '-tle', '-tti']
            ret = subprocess.call(command, shell=True)
        return ret
                ...
                try:
                    if not in_header and not header_parsed and line == "BeginHeader\r\n":
                        ...
                        if line == "EndHeader\r\n":
                            ...
                            if len(columns):
                                event_name = columns[0].replace(' ', '').replace(
                                    '/win:', '/').replace('/Task.', '/')
                                if len(event_name):
                                    column_names[event_name] = columns
                    else:
                        buffer += line
                        # line feeds in the data are escaped.  All real data
                        # lines end with \r\n
                        if len(buffer) and line[-1] != "\r" and buffer[-3:] != "\r\r\n":
                            buffer = buffer.replace("\r\r\n", "\r\n")
                            # pull the event name from the front of the string
                            # so we only do the heavy csv processing for events
                            # we care about
                            comma = buffer.find(',')
                            if comma > 0:
                                event_name = buffer[:comma].replace(' ', '').replace(
                                    '/win:', '/').replace('/Task.', '/')
        ...
        try:
            ...
            for i in xrange(0, length):
                if csv[i] == ',' and not in_quote:
                    ...
                    buffer += csv[i]
                    # quote escaping starts with a quote as the first non-space
    ...
    def ProcessEvents(self, events):
                    ...
                    if navigating and event['name'] == 'Microsoft-IE/Mshtml_CDoc_Navigation/Info':
                        ...
                        if event['name'] == 'Microsoft-Windows-WinINet/WININET_DNS_QUERY/Stop' and id in dns:
                            if 'AddressList' in event['fields']:
                                dns[id]['addresses'] = list(
                                    filter(None, event['fields']['AddressList'].split(';')))
                        ...
                        # Requests
                        # Request created (not necessarily sent) - AddressName
                        # (URL)
                        if event['name'] == 'Microsoft-Windows-WinINet/Wininet_SendRequest/Start':
                            if id not in requests:
                                requests[id] = {}
                            if 'AddressName' in event['fields'] and 'URL' not in requests[id]:
                                requests[id]['URL'] = event['fields']['AddressName']
                        # Headers and size of outbound request - Length,
                        # Headers
                        if event['name'] == 'Microsoft-Windows-WinINet/WININET_REQUEST_HEADER/Info':
                            if id not in requests:
                                requests[id] = {}
                        ...
                        if event['name'] == 'Microsoft-Windows-WinINet/Wininet_SendRequest_Main/Info':
                            if id not in requests:
                                requests[id] = {}
                            requests[id]['start'] = event['ts']
                            if 'Size' in event['fields']:
                                ...
                                if length > 0:
                                    requests[id]['outBytes'] = int(
                                        event['fields']['Size'])
                        # Maps request to source port of connection "Socket" ==
                        # local port
                        if event['name'] == 'Microsoft-Windows-WinINet/Wininet_LookupConnection/Stop':
                            if 'Socket' in event['fields'] and event['fields']['Socket'] in sockets:
                                if id not in requests:
                                    requests[id] = {}
                                connect_id = sockets[event['fields']['Socket']]
                                ...
                                if connect_id in result['sockets']:
                                    ...
                                    result['sockets'][connect_id]['requests'].append(
                                        id)

                        # Headers and size of headers - Length, Headers
                        if event['name'] == 'Microsoft-Windows-WinINet/WININET_RESPONSE_HEADER/Info' and id in requests:
                            ...
                            if 'Length' in event['fields']:
                                requests[id]['inHeadersLen'] = int(
                                    event['fields']['Length'])
                        # inbound bytes (ttfb, keep incrementing end) - Size
                        if event['name'] == 'Microsoft-Windows-WinINet/Wininet_ReadData/Info' and id in requests:
                        ...
                        if event['name'] == 'Microsoft-Windows-WinINet/WININET_STREAM_DATA_INDICATED/Info' and id in requests:  # Size
                            ...
                            if 'start' in requests[id]:
                                ...
                                requests[id]['inPreBytes'] += int(
                                    event['fields']['Size'])
                        # completely finished - URL, Verb, RequestHeaders,
                        # ResponseHeaders, Status, UsageLogRequestCache
                        if event['name'] == 'Microsoft-Windows-WinINet/Wininet_UsageLogRequest/Info' and id in requests:
                        ...
                        # Headers done - Direction changing for capture (no
                        # params)
                        if event['name'] == 'Microsoft-Windows-WinINet/Wininet_SendRequest/Stop' and id in requests:
                            if 'end' not in requests[id]:
                                requests[id]['end'] = event['ts']
                        # raw bytes (before encryption?_) and length -
                        # PayloadByteLength, Payload
                        if event['name'] == 'Microsoft-Windows-WinINet-Capture//' and id in requests:
                            if 'PayloadByteLength' in event['fields'] and 'start' in requests[id]:
                                length = int(event['fields']
                                             ['PayloadByteLength'])
                                if 'end' not in requests[id]:
        ...
        # Calculate some page timings and adjust the start time to the start of
        # actual activity
    ...
    def ProcessResult(self, raw, task):
        ...
        if 'start' in raw['pageData']:
                ...
                try:
                    ...
                    if 'connection' in r and r['connection'] in raw['sockets']:
                        ...
                        if 'start' in c and 'end' in c:
                            request['server_rtt'] = int(
                                round(float(c['end'] - c['start']) / 1000.0))
                        if 'srcPort' in c:
            ...
            try:
                page_data['URL'] = task['url']
                if 'URL' in raw and page_data['URL'] is None:
                    page_data['URL'] = raw['URL']
                if 'loadEventStart' in raw['pageData']:
                    page_data['result'] = 0
                    page_data['loadTime'] = self.Elapsed(
                        raw['pageData']['loadEventStart'])
                    page_data['docTime'] = page_data['loadTime']
                    if 'loadEventEnd' in raw['pageData']:
                        page_data['loadEventStart'] = self.Elapsed(
                            raw['pageData']['loadEventStart'])
                        page_data['loadEventEnd'] = self.Elapsed(
                            raw['pageData']['loadEventEnd'])
                if 'domContentLoadedEventStart' in raw['pageData']:
                    page_data['domContentLoadedEventStart'] = self.Elapsed(
                        raw['pageData']['domContentLoadedEventStart'])
                    if 'domContentLoadedEventEnd' in raw['pageData']:
                        page_data['domContentLoadedEventEnd'] = self.Elapsed(
                            raw['pageData']['domContentLoadedEventEnd'])
                    else:
                ...
                if 'fullyLoaded' in raw['pageData']:
                    page_data['fullyLoaded'] = self.Elapsed(
                        raw['pageData']['fullyLoaded'])
                page_data['cached'] = task['cached']
            except:
                pass
